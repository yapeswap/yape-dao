...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "yapeswap/yapeswap-t2") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const V1_DATA_QUERY = gql`
  query yapeswap($date: Int!, $date2: Int!) {
    current: yapeswap(id: "1") {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    oneDay: yapeswapHistoricalDatas(
    ...
    twoDay: yapeswapHistoricalDatas(
...
export const GLOBAL_CHART = gql`
  query yapeswapDayDatas($startTime: Int!, $skip: Int!) {
    yapeswapDayDatas(
      first: 1000
      skip: $skip
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query yapeswapFactories {
      yapeswapFactories(
